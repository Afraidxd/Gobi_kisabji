...
async def harem(update: Update, context: CallbackContext, page=0) -> None:
    ...
    if not user:
        if update.message:
            await update.message.reply_text('𝙔𝙤𝙪 𝙃𝙖𝙫𝙚 𝙉𝙤𝙩 𝙂𝙧𝙖𝙗 𝙖𝙣𝙮 Car 𝙔𝙚𝙩...')
        else:
            await update.callback_query.edit_message_text('𝙔𝙤𝙪 𝙃𝙖𝙫𝙚 𝙉𝙤𝙩 𝙂𝙧𝙖𝙗 𝙖𝙣𝙮 car 𝙔𝙚𝙩...')
        return
    ...
    for anime, characters in current_grouped_characters.items():
        harem_message += f'\n⥱ <b>{company} {len(characters)}/{await collection.count_documents({"company": company})}</b>\n'
