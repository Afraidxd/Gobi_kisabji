...
async def harem(update: Update, context: CallbackContext, page=0) -> None:
    ...
    if not user:
        if update.message:
            await update.message.reply_text('Go and Grab Some car First....')
        else:
            await update.callback_query.edit_message_text('Go and Grab Some car First..')
        return
    ...
    for anime, characters in current_grouped_characters.items():
        ...
        for character in characters:
            ...
            harem_message += f'{character["id"]} {character["car name"]} ×{count}\n'

    total_count = len(user['characters'])

    keyboard = [[InlineKeyboardButton(f"See Garage ({total_count})", switch_inline_query_current_chat=f"collection.{user_id}")]]
    ...
    if 'favorites' in user and user['favorites']:
        ...
        if user['characters']:
            ...
            if update.message:
                await update.message.reply_text("Your Garage is Empty :)")
...
async def harem_callback(update: Update, context: CallbackContext) -> None:
    ...
    if query.from_user.id != user_id:
        await query.answer("It's Not Your Garage", show_alert=True)
        return
