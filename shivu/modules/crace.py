...
async def participate(update, context):
    update = context.job.context['update'] if hasattr(context, 'job') else update
...
async def timeout_race(context):
    ...
    if len(config['participants']) < 2:
        await context.bot.send_message(update.message.chat_id, "❌ Not enough participants to start the race.")
        config['participants'] = []
        return
...
async def remind_to_join(context):
    ...
    await context.bot.send_message(update.message.chat_id, "🏁 Join the race before time runs out! 🏎️")
